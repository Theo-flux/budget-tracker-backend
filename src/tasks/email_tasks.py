...
def send_email_verification_task(*args, **kwargs):
    ...
    try:
        loop.run_until_complete(Mailer.send_email_verification(email=email, first_name=first_name))
    finally:
        loop.close()
...
def send_password_reset_task(*args, **kwargs):
    ...
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(Mailer.send_password_reset(email=email, first_name=first_name))
    finally:
        loop.close()
